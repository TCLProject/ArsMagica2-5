...
public class TransformerNatural extends TransformerSingleBlock<NBTNone>
{
    ...
    {
        this(BlockMatcher.of(AMCore.specialRegistry, BlocksCommonProxy.genericSolid, 0), DEFAULT_NATURAL_EXPANSION_DISTANCE, DEFAULT_NATURAL_EXPANSION_RANDOMIZATION);
    }
    ...
    public TransformerNatural(String sourceMatcherExpression, double naturalExpansionDistance, double naturalExpansionRandomization)
    {
        ...
        this.naturalExpansionDistance = naturalExpansionDistance + 3;
...
//                        double yForDistance = coord.y * 0.3 + currentY * 0.7;
                        double distToOrigSQ = getDistanceSqXZ(coord.x, coord.z, currentX, currentZ); // stop the strange y cutoffs
    ...
    private double getDistanceSqXZ(double x1, double z1, double x, double z) {
        double dx = x1 - x;
        double dz = z1 - z;
        return dx * dx + dz * dz;
    }
