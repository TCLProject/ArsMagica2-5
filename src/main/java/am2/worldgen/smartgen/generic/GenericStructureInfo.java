...
import am2.worldgen.smartgen.generic.gentypes.MazeGenerationInfo;
...
import am2.worldgen.smartgen.reccomplexutils.*;
import am2.worldgen.smartgen.reccomplexutils.json.JsonUtils;
import am2.worldgen.smartgen.reccomplexutils.json.NbtToJson;
import am2.worldgen.smartgen.struct.info.*;
import am2.worldgen.smartgen.struct.inventory.InventoryGenerationHandler;
...
public class GenericStructureInfo implements StructureInfo<GenericStructureInfo.InstanceData>, Cloneable
{
    ...
    public static GenericStructureInfo createDefaultStructure()
    {
        ...
        genericStructureInfo.transformers.add(new TransformerNaturalAir(BlockMatcher.of(AMCore.specialRegistry, BlocksCommonProxy.genericSpace, 1), TransformerNaturalAir.DEFAULT_NATURAL_EXPANSION_DISTANCE, TransformerNaturalAir.DEFAULT_NATURAL_EXPANSION_RANDOMIZATION));
        genericStructureInfo.transformers.add(new TransformerNegativeSpace(BlockMatcher.of(AMCore.specialRegistry, BlocksCommonProxy.genericSpace, 0)));
        genericStructureInfo.transformers.add(new TransformerNatural(BlockMatcher.of(AMCore.specialRegistry, BlocksCommonProxy.genericSolid, 0), TransformerNatural.DEFAULT_NATURAL_EXPANSION_DISTANCE, TransformerNatural.DEFAULT_NATURAL_EXPANSION_RANDOMIZATION));
        genericStructureInfo.transformers.add(new TransformerReplace(BlockMatcher.of(AMCore.specialRegistry, BlocksCommonProxy.genericSolid, 1)).replaceWith(new WeightedBlockState(null, BlockStates.fromMetadata(Blocks.air, 0), "")));
    ...
    public void generate(final StructureSpawnContext context, InstanceData instanceData)
    {
        ...
        if (!context.generateAsSource)
        {
            for (Pair<Transformer, NBTStorable> pair : transformers)
            {
                ...
                if (transformer.generatesInPhase(transformerData, Transformer.Phase.BEFORE) && transformer.getDisplayString().startsWith("Natural Air:")) { // air goes first
                    transformer.transform(transformerData, Transformer.Phase.BEFORE, context, worldData, transformers);
                }
            }
        }

        if (context.solidReplacerCounter >= AMCore.config.getMaxDisplacedBlocks()) return; // "stop the generation, this is stupid"
        else {
            for (int[] coords: context.arrayListToSetToAir) {
                context.setBlock(coords[0], coords[1], coords[2], BlockStates.defaultState(Blocks.air));
            }
        }
        ...
        if (!context.generateAsSource)
        {
            for (Pair<Transformer, NBTStorable> pair : transformers)
            {
                Transformer transformer = pair.getLeft();
                NBTStorable transformerData = pair.getRight();
                if (transformer.generatesInPhase(transformerData, Transformer.Phase.BEFORE) && !transformer.getDisplayString().startsWith("Natural Air:")) {
                    transformer.transform(transformerData, Transformer.Phase.BEFORE, context, worldData, transformers);
                }
            }
        }
        ...
        for (int pass = 0; pass < 2; pass++)
        {
            for (BlockCoord sourceCoord : blockCollection)
            {
                ...
                if (context.includes(worldPos) && AMCore.specialRegistry.isSafe(state.getBlock()))
                {
...
//                      if (context.generateAsSource || !(tileEntity instanceof GeneratingTileEntity) || ((GeneratingTileEntity) tileEntity).shouldPlaceInWorld(context, instanceData.tileEntities.get(sourceCoord)))
                        if (true) // !(tileEntity instanceof GeneratingTileEntity) will always be true
                        {
                            if (context.setBlock(worldPos, state) && world.getBlock(worldPos.x, worldPos.y, worldPos.z) == state.getBlock())
                            {
                                if (tileEntity != null && AMCore.specialRegistry.isSafe(tileEntity))
                                {
                                    ...
                                    if (!context.generateAsSource)
                                    {
                                        if (tileEntity instanceof IInventory)
                                        {
                                            ...
                                            InventoryGenerationHandler.generateAllTags(inventory, AMCore.specialRegistry.itemHidingMode(), random);
                                        }
                                    }
                                }
...
//            tileEntities.entrySet().stream().filter(entry -> entry.getValue() instanceof GeneratingTileEntity).forEach(entry -> ((GeneratingTileEntity) entry.getValue()).generate(context, instanceData.tileEntities.get(entry.getKey())));
        ...
        {
            AMCore.logger.warn("Structure generated with over " + MAX_GENERATING_LAYERS + " layers; most likely infinite loop!");
        }
    }
...
//            });
    ...
    {
        return new IvWorldData(worldDataCompound, world, AMCore.specialRegistry.itemHidingMode());
    }
    ...
    public static class Serializer implements JsonDeserializer<GenericStructureInfo>, JsonSerializer<GenericStructureInfo>
    {
        public GenericStructureInfo deserialize(JsonElement jsonElement, Type par2Type, JsonDeserializationContext context)
        {
            ...
            else
            {
                ...
                AMCore.logger.warn("Structure JSON missing 'version', using latest (" + LATEST_VERSION + ")");
            }
        ...
        public JsonElement serialize(GenericStructureInfo structureInfo, Type par2Type, JsonSerializationContext context)
        {
            ...
            if (!AMCore.USE_ZIP_FOR_STRUCTURE_FILES && structureInfo.worldDataCompound != null)
            {
