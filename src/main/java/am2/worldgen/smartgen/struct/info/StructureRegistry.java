...
import am2.worldgen.smartgen.StructureSelector;
import am2.worldgen.smartgen.generic.GenericStructureInfo;
import am2.worldgen.smartgen.generic.gentypes.*;
import am2.worldgen.smartgen.generic.transformers.Transformer;
import am2.worldgen.smartgen.reccomplexutils.CustomizableBiMap;
...
import am2.worldgen.smartgen.struct.villages.GenericVillageCreationHandler;
import am2.worldgen.smartgen.struct.villages.TemporaryVillagerRegistry;
...
//        RCEventBus.INSTANCE.post(event);
        ...
        if (GenerationConstants.shouldStructureLoad(key, domain))
        {
            String baseString = allStructures.put(key, info, custom) != null ? "Replaced structure '%s'" : "Registered structure '%s'";
            AMCore.logger.info(String.format(baseString, key));

            structureData.put(key, new StructureData(false, domain), custom);
        ...
        if (needsGenerationCacheUpdate)
        {
            ...
            for (Map.Entry<String, StructureInfo> entry : allStructures.getMap().entrySet())
            {
                ...
                if (!structureData.disabled
                        && GenerationConstants.shouldStructureGenerate(key, structureData.domain)
