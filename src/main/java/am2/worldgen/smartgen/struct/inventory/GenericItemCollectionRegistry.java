...
import am2.worldgen.smartgen.struct.inventory.GenericItemCollection.Component;
...
public class GenericItemCollectionRegistry
{
    ...
    {
        if (GenerationConstants.shouldInventoryGeneratorLoad(key, domain))
        {
    ...
    private void clearCaches()
    {
        ...
        for (Map.Entry<String, Component> entry : allComponents.getMap().entrySet())
        {
            ...
            if (!structureData.disabled
                    && GenerationConstants.shouldInventoryGeneratorGenerate(key, structureData.domain)
