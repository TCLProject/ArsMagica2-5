...
// huge courtesy of CNPCs+ author, KAMKEEL
public class EntityProjectile  extends EntityThrowable {
    ...
    public EntityProjectile(World par1World, EntityLivingBase par2EntityLiving, ItemStack item, boolean isNPC)
    {
        ...
        if (isNPC) {
            this.generic = (EntityGeneric) this.thrower;
            this.getStatPropertiesFromValues(this.generic.getValues());
        }
    }
...
//    			}
    ...
    public void getStatPropertiesFromValues(HashMap<String, String> stats)
    {
        this.damage = Float.parseFloat(stats.get("RattD"));
        this.punch = Integer.parseInt(stats.get("RattK"));
        if (stats.containsKey("projAccel")) { // projectile properties
            this.accelerate = Boolean.parseBoolean(stats.get("projAccel"));
            this.explosive = Boolean.parseBoolean(stats.get("projExpl"));
            this.explosiveRadius = Integer.parseInt(stats.get("projExplRad"));
            this.effect = EnumPotionType.values()[Integer.parseInt(stats.get("projEff"))];
            this.duration = Integer.parseInt(stats.get("projDur"));
            this.amplify = Integer.parseInt(stats.get("projAmp"));
            this.setParticleEffect(EnumParticleType.values()[Integer.parseInt(stats.get("projTrail"))]);
            this.dataWatcher.updateObject(23, Integer.valueOf(stats.get("projSize")));
            this.dataWatcher.updateObject(24, Byte.valueOf((byte) (Boolean.parseBoolean(stats.get("projGlows")) ? 1 : 0)));
            this.setSpeed(Integer.parseInt(stats.get("projSpeed")));
            this.setHasGravity(Boolean.parseBoolean(stats.get("projGrav")));
            setIs3D(Boolean.parseBoolean(stats.get("proj3D")));
            this.setRotating(Boolean.parseBoolean(stats.get("projSpin")));
            this.setStickInWall(Boolean.parseBoolean(stats.get("projStick")));
        }
    }
