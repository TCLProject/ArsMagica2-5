...
public class EntityAIOrbitTarget extends EntityAIBase
{
    ...
    public EntityAIOrbitTarget(EntityGeneric par1EntityCreature, double par2, float par4, boolean par5)
    {
    ...
    public boolean shouldExecute()
    {
        ...
        else
        {
        	...
        	return d0 >= this.distance / 2.0F && (this.theEntity.invvalues.get("proj") == null ? d0 <= this.distance : true);
        }
    }
    ...
    public void updateTask()
    {
    	...
    	if (this.theEntity.getNavigator().noPath() && this.tick >= 0)
    	{
    		if (this.theEntity.onGround && !this.theEntity.isInWater())
    		{
	            ...
	            this.tick = MathHelper.ceiling_double_int(this.theEntity.getDistance(this.movePosX, this.movePosY, this.movePosZ) / (Double.valueOf(this.theEntity.getValue("movS")) / 20.0F));
	            if (this.decay)
	            {
