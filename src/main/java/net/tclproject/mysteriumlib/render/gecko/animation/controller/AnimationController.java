...
import net.tclproject.mysteriumlib.render.gecko.animation.keyframe.*;
import net.tclproject.mysteriumlib.render.gecko.animation.render.AnimatedModelRenderer;
import net.tclproject.mysteriumlib.render.gecko.animation.snapshot.BoneSnapshot;
import net.tclproject.mysteriumlib.render.gecko.animation.snapshot.BoneSnapshotCollection;
import net.tclproject.mysteriumlib.render.gecko.easing.EasingManager;
...
import net.tclproject.mysteriumlib.render.gecko.event.CustomInstructionKeyframeEvent;
import net.tclproject.mysteriumlib.render.gecko.event.ParticleKeyFrameEvent;
import net.tclproject.mysteriumlib.render.gecko.event.SoundKeyframeEvent;
import net.tclproject.mysteriumlib.render.gecko.reload.ReloadManager;
...
public abstract class AnimationController<T extends IAnimatedEntity>
{
	 ...
	 * @param name                  Name of the animation controller (move_controller, size_controller, attack_controller, etc.)
	 * @param transitionLengthTicks How long it takes to transition between animations (IN TICKS!!)
	 * @param customEasingMethod    If you want to use an easing method that's not included in the EasingType enum, pass your method into here. The parameter that's passed in will be a number between 0 and 1. Return a number also within 0 and 1. Take a look at {@link EasingManager}
	 */
	...
	public void process(double tick, AnimationTestEvent animationTestEvent, List<AnimatedModelRenderer> modelRendererList, BoneSnapshotCollection boneSnapshotCollection)
	{
		...
		assert tick >= 0 : "arsmagica2: Tick was less than zero";
