...
import net.tclproject.mysteriumlib.asm.core.MMiscUtils.LogHelper;
...
public class MTargetClassTransformer {
	/**Instance of MMiscUtils needed in order to make a logger.*/
	static MMiscUtils utils = new MMiscUtils();
	/**System logger that this class uses (uses System.out)*/
	public static LogHelper logger = utils.new SystemLogHelper();
	/**Map of "target class name":"List of MASMFix/es to be applied".*/
    protected HashMap<String, List<MASMFix>> fixesMap = new HashMap<String, List<MASMFix>>();
    /**Class that will parse the fix class and methods.*/
    private MFixParser containerParser = new MFixParser(this);
    /**MetaReader instance used.*/
    protected MetaReader metaReader = new MetaReader();

    /**Adds a fix to the list to be inserted.*/
    public void registerFix(MASMFix fix) {
        if (fixesMap.containsKey(fix.getTargetClassName())) {
            ...
            List<MASMFix> list = new ArrayList<MASMFix>(2); // Create a new list of fixes to be applied to the class.
    ...
    public byte[] transform(String className, byte[] classBytes) {
        List<MASMFix> fixes = fixesMap.get(className); // gets the fixes for the class

        if (fixes != null) { // if there are any
            Collections.sort(fixes); // sort fixes using method inside MASMFix
            ...
            for (MASMFix notInserted : fixes) { // since inserted fixes get removed, we can just iterate through ones left
